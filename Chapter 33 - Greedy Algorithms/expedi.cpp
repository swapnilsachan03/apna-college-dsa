...
Lines 2 to N+1: Each line contains two space-separated integers describing a fuel stop: The first integer is the distance from the town to the stop; the second is the amount of fuel available at that stop.
...
int main()
{
  ...
  while (t--) {
    ...
    for (int i = 0; i < n; i++) {
      ...
      pq.push(towns[i].second);
    }

    if (cantDo) {
      cout << "Can not reach town" << endl;
      continue;
    }
    ...
    cout << endl << "Minimum nmuber of stops: " << ans << endl;
  }
